...
\if 0\ICANN 
{\color{blue} The promise of artificial vision to identify objects in natural images is ever increasing. }
\fi
...
\if 0\ICANN {\color{blue} The main difficulty of this task is to learn to categorize this particular object class given all possible spatial configurations and respective geometrical visual transformations. }
\fi
...
\if 0\ICANN {\color{blue} This last implementation is particularly interesting for our sake as it predicts in the image the probability of proposed bounding boxes around the visual object. While rapid, the number of boxes greatly increases with image size and necessitates dedicated hardware. }
\fi
...
\if 0\ICANN
{\color{blue} Herein, we believe that this fact is an essential factor to reproduce and understand this active vision process.}
\fi
...
Following such an active inference scheme~\citep{Mirza18} 
%and simulations of the resulting optimization 
numerical simulations reproduce sequential eye movements that fit well with empirical data. %Compared to~\citet{Najemnik05}, 
...
% TODO : integrate this paragraph to that oriented on computer vision
...
\if 0\ICANN {\color{blue}\paragraph{Outline.}}\fi
...
Stemming from the active vision general principles, our aim is to produce a principled model that may both explain the essential features of human vision and provide ways toward efficient computer implementations. \if 0\ICANN
{\color{blue}\emph{We also aim at reunifying the fragmentation of the many different approaches respective to their fields (Machine learning, neuroscience, robotics), and envisage an integrated computational model of foveated active vision.}}
\fi
...
\if 0\ICANN {\color{blue}In addition, the agent is equipped with a foveated sensor, %and with the ability to actively scan the visual image as defined by a generative (internal) modelWe will use this constraint as an asset 
