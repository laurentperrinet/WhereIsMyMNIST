...
The search experience is formalized and simplified in a way reminiscent to classical psychophysic experiments: an observer is asked to classify digits (for instance as taken from the MNIST database) as they are shown on a computer display.
...
Indeed, in order to analyze a complex visual scene, there are two types of processing that need to be done. On the one side, you need  to analyze in detail what is at the center of fixation, that is the region of interest currently processed. On the other side, you also need to analyze the surrounding part, even if the resolution is low, in order to choose what is the next center of fixation. This basically means making a choice of “what’s interesting next”. You do not necessarily need to know what it is, but you need to that it’s interesting enough, and of course you need to know what action to take to move the center of fixation at the right position. This is reminiscent of the What/Where separate visual processing separation observed in monkeys and humans the ventral and dorsal visual pathways \citep{mishkin1983object}.
...
It is also assumed that a set of motor commands $A = \{..., a, ...\}$ (here saccades) may control the body posture, but not the object's identity, so that $y$ is invariant to $a$. Actuating a command $a$ changes the viewpoint to $u'$, which fills the system with a new visual sample $x'\sim p(X|u', y)$. The more viewpoints you have, the more certain you are about the object identity through a chain rule sequential evidence accumulation. 

In an optimal search setup however \citep{Najemnik05}, you need to choose the next viewpoint that will help you \emph{the most} to disambiguate the scene.
...
\begin{figure}[t!]%%[p!]
	...
	\caption{%
		...
		\A The visual input is constructed the following way: first a  $128\times 128$  natural-like background noise is generated, characterized by noise contrast, mean spatial frequency and bandwidth~\citep{Sanz12}. Then a circular mask is put on. Last a sample digit is selected from the MNIST database (of size $28\times 28$), rectified, multiplied by a contrast factor and overlayed on the background at a random position (see an example in Figure~\ref{fig:intro}-A, \DIS ). %
...
This image is passed unmodified to the agent's visual categorical pathway (the ``What'' pathway), that is realized by a convolutional neural network, here  the known ``LeNet'' classifier~\citep{Lecun1998}. The network structure, that processes the input to identify the target category, is  provided (and unmodified) by the pyTorch library~\citep{Paszke17}.
It is made of a 3 convolution layers followed by two fully-connected layers.
%. It is trained over the (centered) MNIST database after approx $20$ training epochs. %Input images are rectified (\emph{with a mean and standard deviation of respectively $0.1307$ and $0.3081$ {\bf why??}}).} \fi 
...
%This strategy achieves an average $98.7\%$ accuracy on the validation dataset~\citep{Lecun1998}.%

\begin{figure}[t!]%%[p!]
	\centering{\includegraphics[width=\linewidth]{fig_what_accmap.png}}
	\caption{\A Example input samples from the ``What'' training set, with SNR randomly set between 0.3 and 0.7, and target shift randomly set using a Gaussian bivariate spatial offset with a standard deviation of 15 pixels.
		\B $55 \times 55$ shift-dependent accuracy map, measured for different target eccentricities on the test set after training.  
	}
\label{fig:accuracy}
\end{figure}

A specific dataset is constructed to train the network. It is made of  randomly shifted/randomly attenuated digits overlayed over a noisy background, as defined above. Both the offset, the contrast and the background noise render the task more difficult. The relative contrast of the digit is randomly set between 0.3 and 0.7.  The network is trained incrementally by progressively increasing the offset variability (of a bivariate central gaussian) by increasing the standard deviation from 0 to 15 (with a maximal offset set at 25 pixels). The shift and background are re-generated at each epoch. The shift standard deviation increases of one unit every 5 epochs. The network is trained on a total of 75 epochs, with 60000 examples generated at each epoch from the MNIST original training set. Note that at the end of the training, many digits fall outside the center of the fovea, so that many examples are close to impossible to classify, either because of a low contrast or a too large eccentricity. At the end of the training process, the average accuracy is thus of 34\% (though it had a 91\% accuracy after the 5th epoch, when the digits were only at the center). 

After training, a shift-dependent accuracy map is computed by systematically testing the network accuracy on every horizontal and vertical offset on a set of 1000 samples generated from the MNIST test set, with an offset amplitude of 27 pixels (see figure \ref{fig:accuracy}).  This forms a $55\times 55$ accuracy map showing higher accuracy at the center, and a slow decreasing accuracy with target eccentricity (with over 70\% accuracy plateau showing a shift invariance on a 7-8 pixels eccentricity radius). This significant shift invariance is a known effect of convolutional computation, that is obtained here at the cost of a lesser central recognition rate (around 80\%), remembering the classification task is here harder by construction. The accuracy fastly drops fir greater than 10 pixels eccentricity, reaching the baseline 10\% chance level at around 20 pixels offset. 
%, as shown in(with a central value of $80\%$ corresponding to the validation score without any translational shift),




\subsection{Retinal transform: Peripheral vision and log Polar encoding}
%
% page 476 de :
%
...
{\color{magenta} [TODO] What training, translation invariance and robust accuracy map.}
