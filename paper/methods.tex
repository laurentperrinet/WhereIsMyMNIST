...
with $x$ the visual field in our case. Assume now that the cause $e$ of the visual field splits in two (independent) components, namely $e = (u,y)$ with $u$ the body posture (in our case the gaze orientation) and $y$ the object shape. Assume also that a set of motor commands $A = \{..., a, ...\}$ may control the body posture, but not the object's shape, so that $y$ is invariant to $a$. Active inference assumes that one can both estimate $u$ and $y$ through predicting the effect of every $a$ in $A$ and choosing the action that optimizes future inferences through a certain metric (future accuracy, future posterior entropy, future variational free energy, ...), given a current inference $p(U,Y|x)$. Estimating the effect of every action over the range of every possible object shapes and body postures is combinatorially hard, even in simple cases such as vision, and thus infeasible in practice. 

There are however many shortcuts allowing to render the calculation amenable. This includes sparse encoding, approximate inference through model separation, sampling and parametric training. 
