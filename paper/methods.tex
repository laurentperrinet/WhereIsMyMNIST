...
`What'' and the ``Where'' pathways by analogy with the brain {\bf [ref needed]}. \emph{Note that %from the retinotopic projection of the visual information, 
...
The texture is designed to fit well with the statistics of natural images. We chose an isotropic setting where textures are characterized by solely two parameters. One controls the median spatial frequency $sf_0$ of the noise, while the other controls the bandwidth around the central frequency. Equivalently, this can be considered as the band-pass filtering of random white noise image. Finally, these images are rectified to have a normalized contrast.
