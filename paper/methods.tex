...
\if 1\ICANN 
In principle, active inference predicts future inferences through modeling the full sequence of operations that lead to both estimate $p(U|x)$ and $p(Y|x)$. This means predicting the future visual fields over all possible saccades, which may be exaggeratedly computationally intensive.
...
The network is trained to predict the distribution $\boldsymbol{a}$ knowing the retinal input $\boldsymbol{x}$ by comparing it to the known ground truth distribution computed over the collicular map. As a loss function, we will naturally use the Kullback-Leibler divergence between the ground truth and the predicted map.
