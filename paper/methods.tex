...
This image is passed unmodified to the agent's visual categorical pathway (the ``What'' pathway), that is realized by a convolutional neural network, here the well-known ``LeNet'' classifier~\cite{Lecun1998}. The network structure, that processes the input to identify the target category,% is  provided (and unmodified) by the Pytorch library~\cite{NEURIPS2019_9015}.
%It
 is made of 3 convolution layers followed by two fully-connected layers.
