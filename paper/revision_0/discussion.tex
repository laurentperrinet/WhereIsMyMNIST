...
%The translation invariance found in convolutional processing may thus be extended to both rotation and scale invariance in the log-polar domain. Incorporating this scale and rotation invariance may thus extend the generalization capabilities of the model.
...
%The only feasible comparisons are those with non-focal image processing/computer vision methods, in our case state-of-the art convolutional neural networks, that we provide in figs 4, 5 and 6 (as our central vision is implemented this way). }


%{\color{magenta} Our model is, to our best knowledge, the first case of a bio-realistic log-polar implementations of an active vision framework.}
...
%If some conclusions are similar (like the sub-linear cost of a sequential/foveal processing), this does not preclude originality and novelty of our method.
...
%If real world computer vision is an objective we address, and probably the most important issue in foveal image processing, %i.e. attaining YOLO-like performances on large images with fovea-based processing.
