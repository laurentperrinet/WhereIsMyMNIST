...
FIC_NAME = '2019-01-10-multi-1-withbias'
EPOCHS = 1500
...
class Net(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_hidden1 = nn.Conv3d(2, 20, [64, 1, 1], bias = BIAS_CONV, stride=1, padding=0)
        self.conv_hidden2 = nn.Conv3d(1, 2, [20, 1, 1], bias = BIAS_CONV, stride=1, padding=0)
        self.deconv_hidden2 = nn.Conv3d(1, 20, [2, 1, 1], bias = BIAS_CONV, stride=1, padding=0)
        N_out = N_azimuth // 6 * N_eccentricity // 4
        self.deconv_hidden1 = nn.Conv3d(1, N_out, [20, 1, 1], bias = BIAS_CONV, stride=1, padding=0)
    ...
    def forward(self, image):
        ...
        # in
        data = self.pool(F.relu(self.conv1(image)))
        data = self.pool(F.relu(self.conv2(data)))
        
        data = data.permute(0, 2, 1, 3, 4)    # shape ?, 2, 64, 3, 2
        data = F.relu(self.conv_hidden1(data))  # shape ?, 10, 1, 3, 2
        data = data.permute(0, 2, 1, 3, 4)  # shape ?, 1, 10, 3, 2
        data = F.dropout(data, p = .5) 
        data = self.conv_hidden2(data) # shape ?, 1, 1, 3, 2
        data = data.permute(0, 2, 1, 3, 4) # shape ?, 1, 1, 3, 2
        data = F.relu(self.deconv_hidden2(data)) # shape ?, 10, 1, 3, 2
        data = data.permute(0, 2, 1, 3, 4) # shape ?, 1, 10, 3, 2
        data = self.deconv_hidden1(data) # shape ?, N_out, 1, 3, 2
        data = data.permute(0, 2, 1, 3, 4) # shape ?, 1, N_out, 3, 2
        
        # out
        for i in range(6): # Azimuth
            for j in range(4): # Eccentricity
                data_cell = data[:,:,:,i,j].view(-1, N_azimuth // 6, N_eccentricity // 4) 
                if j == 0 :
                    data_row = data_cell
                else:
...
def test(net, minibatch_size, optimizer=optimizer,
    ...
    prediction = net(batch['image_white'].float().to(device))
