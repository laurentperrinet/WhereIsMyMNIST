...
FIC_NAME = '2019-01-09-multi-1-withbias'
EPOCHS = 1500
...
class Net(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.conv_hidden1 = nn.Conv3d(1, 20, [256, 1, 1], bias = BIAS_CONV, stride=1, padding=0)
        ...
        N_out = N_azimuth // 3 * N_eccentricity // 2
    ...
    def forward(self, image):
        ...
        data = data.permute(0, 2, 1, 3, 4)    # shape ?, 1, 256, 3, 2
        data = F.relu(self.conv_hidden1(data))  # shape ?, 20, 1, 3, 2
        data = data.permute(0, 2, 1, 3, 4)  # shape ?, 1, 20, 3, 2
        data = F.dropout(data, p = .5) 
        data = self.conv_hidden2(data) # shape ?, 2, 1, 3, 2
        data = data.permute(0, 2, 1, 3, 4) # shape ?, 1, 2, 3, 2
        data = F.relu(self.deconv_hidden2(data)) # shape ?, 20, 1, 3, 2
        data = data.permute(0, 2, 1, 3, 4) # shape ?, 1, 20, 3, 2
